class LasPantallas {
  constructor() { 
    ...
    this.pantallas = new Array(16).fill(0);
  ...
  draw() {
    if (this.pantallas[0] == 0) {
    ...
    } else if (this.pantallas[6] == 0) {//juego
      ...
      this.boton.dibujar("morir",50,50);
    } else if (this.pantallas[9] == 0) {//ganaste
    ...
    } else if (this.pantallas[13] == 0) {
      this.labase("LOS HERMANOS ASUSTADOS DECIDIERON ABANDONAR EL PLAN DE VIVIR EN EL BOSQUE Y VUELVEN CON SU MAMÁ", this.foto[14]);
    } else if (this.pantallas[14] == 0) {
      this.labase("", this.foto[15]);
      this.boton.dibujar("menu",50,50);
    } else if (this.pantallas[15] == 0) {//perdiste
      this.labase("EL LOBO VENCIÓ Y AHORA SE PREPARARÁ UNA RICA CENA CON SU PRESA", this.foto[16]);
      this.boton.dibujar("menu",50,50);
    }
  }
  mousePressed() {
    if (this.pantallas[0] == 0 && this.boton.areaBoton(500,550)) { //derecho
      for (let i = 0; i < 15; i++) {
        this.pantallas[i] = 1;
        this.pantallas[1] = 0;
      }
    } else if (this.pantallas[0] == 0 && this.boton.areaBoton(50,50)) { //izquierdo
      for (let i = 0; i < 15; i++) {
        this.pantallas[i] = 1;
        this.pantallas[14] = 0;
      }
    } else if (this.pantallas[1] == 0 && this.boton.areaBoton(500,550)) { //derecho
      for (let i = 0; i < 15; i++) {
        this.pantallas[i] = 1;
        this.pantallas[2] = 0;
      }
    } else if (this.pantallas[2] == 0 && this.boton.areaBoton(500,550)) {//derecho
      for (let i = 0; i < 15; i++) {
        this.pantallas[i] = 1;
        this.pantallas[3] = 0;
      }
    } else if (this.pantallas[2] == 0 && this.boton.areaBoton(50,50)) { //izquierdo
      for (let i = 0; i < 15; i++) {
        this.pantallas[i] = 1;
        this.pantallas[13] = 0;
      }
    } else if (this.pantallas[3] == 0 && this.boton.areaBoton(500,550)) { //derecho
      for (let i = 0; i < 15; i++) {
        this.pantallas[i] = 1;
        this.pantallas[4] = 0;
      }
    } else if (this.pantallas[4] == 0 && this.boton.areaBoton(500,550)) { //derecho
      for (let i = 0; i < 15; i++) {
        this.pantallas[i] = 1;
        this.pantallas[5] = 0;
      }
    } else if (this.pantallas[4] == 0 && this.boton.areaBoton(50,50)) { //izquierdo
      for (let i = 0; i < 15; i++) {
        this.pantallas[i] = 1;
        this.pantallas[15] = 0;
      }
    } else if (this.pantallas[5] == 0 && this.boton.areaBoton(500,550)) { //derecho
      for (let i = 0; i < 15; i++) {
        this.pantallas[i] = 1;
        this.pantallas[6] = 0;

      }
    } else if (this.pantallas[6] == 0 && this.boton.areaBoton(500,550)) { //derecho
      this.juegoSet = true;
    } else if (this.pantallas[6] == 0 && this.boton.areaBoton(50,50)) { //izquierdo
      for (let i = 0; i < 15; i++) {
        this.pantallas[i] = 1;
        this.pantallas[15] = 0;
      }
    } else if (this.pantallas[6] == 0) {
      if (mouseX >= 250 && mouseX <= 350 && mouseY >= 410 && mouseY <= 450) {
        this.juegoSet = false;
        this.juegoActivo = true;
      }
    }  else if (this.pantallas[7] == 0 && this.boton.areaBoton(500,550)) {//derecho
      for (let i = 0; i < 15; i++) {
        this.pantallas[i] = 1;
        this.pantallas[8] = 0;
      }
    } else if (this.pantallas[8] == 0 && this.boton.areaBoton(500,550)) {//derecho
      for (let i = 0; i < 15; i++) {
        this.pantallas[i] = 1;
        this.pantallas[9] = 0;
      }
    } else if (this.pantallas[8] == 0 && this.boton.areaBoton(50,50)) { //izquierdo
      for (let i = 0; i < 15; i++) {
        this.pantallas[i] = 1;
        this.pantallas[15] = 0;
      }
    } else if (this.pantallas[9] == 0 && this.boton.areaBoton(500,550)) {//derecho
      for (let i = 0; i < 15; i++) {
        this.pantallas[i] = 1;
        this.pantallas[10] = 0;
      }
    } else if (this.pantallas[10] == 0 && this.boton.areaBoton(500,550)) { //derecho
      for (let i = 0; i < 15; i++) {
        this.pantallas[i] = 1;
        this.pantallas[11] = 0;
      }
    } else if (this.pantallas[11] == 0 && this.boton.areaBoton(500,550)) { //derecho 
      for (let i = 0; i < 15; i++) {
        this.pantallas[i] = 1;
        this.pantallas[12] = 0;
      }
    } else if (this.pantallas[12] == 0 && this.boton.areaBoton(500,550)) { //derecho 
      for (let i = 0; i < 15; i++) {
        this.pantallas[i] = 1;
        this.pantallas[0] = 0;
      }
    } else if (this.pantallas[13] == 0 && this.boton.areaBoton(50,50)) { //izquierdo
      for (let i = 0; i < 15; i++) {
        this.pantallas[i] = 1;
        this.pantallas[0] = 0;
      }
    } else if (this.pantallas[14] == 0 && this.boton.areaBoton(50,50)) {  //izquierdo
      for (let i = 0; i < 15; i++) {
        this.pantallas[i] = 1;
        this.pantallas[0] = 0;
      }
    } else if (this.pantallas[15] == 0 && this.boton.areaBoton(50,50)) { //izquierdo
      for (let i = 0; i < 15; i++) {
        this.pantallas[i] = 1;
        this.pantallas[0] = 0;
      }
